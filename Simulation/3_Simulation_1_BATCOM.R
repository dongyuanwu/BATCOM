...
for (nset in 1:length(simdat)) {
    ...
    beta.cov <- c(10000, rep(0.01, ncelltype*ncelltype), rep(1, nspot*2))
    
    for(k in 1:nsim) {
        
        print(paste0("ncelltype=", ncelltype, "; rho=", simset$rho, 
                     "; phi=", simset$phi, "; p=", simset$p, "; sprate=", 
                     simset$sprate, "; iter=", k))
        ...
        re <- params[-c(1:(ncelltype*ncelltype+3))]
        reL <- re[1:(length(re)/2)][spot[, 1]]
        reR <- re[(length(re)/2+1):length(re)][spot[, 2]]
        params <- params[1:(ncelltype*ncelltype+3)]
...
# save results
saveRDS(finalsim, "simres1_batcom.rds")
